-- Repair operational RPCs used by the realtime server on startup.
-- This keeps production databases that missed or partially applied older migrations healthy.
...
DROP FUNCTION IF EXISTS public.award_daily_chips();

CREATE OR REPLACE FUNCTION public.award_daily_chips()
RETURNS TABLE(player_id UUID, new_balance BIGINT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN QUERY
  UPDATE public.profiles p
  SET chip_balance = p.chip_balance + 2000,
      broke_at = NULL
  WHERE p.chip_balance = 0
    AND p.broke_at IS NOT NULL
    AND p.broke_at <= now() - INTERVAL '24 hours'
  RETURNING p.id, p.chip_balance;
END;
$$;
